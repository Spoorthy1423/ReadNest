...
export default function Create() {
  ...
  const handleSubmit = async () => {
    ...
    if (!imageBase64) missing.push("Image");
    if (missing.length) {
    ...
    try {
      ...
      // Build a data URL that Cloudinary accepts reliably (especially on web where URIs can be blob:)
      const uri = image ?? "";
      const uriParts = uri.split(".");
      const lastPart = (uriParts[uriParts.length - 1] || "").toLowerCase();
      const knownExts = new Set(["jpg", "jpeg", "png", "webp", "gif", "heic"]);
      const ext = knownExts.has(lastPart) ? lastPart : "jpeg";
      const imageType = ext === "jpg" ? "image/jpeg" : `image/${ext}`;
